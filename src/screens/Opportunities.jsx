...
import { Card, Badge, Button, Input, Select, Checkbox } from '../components';
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        <span style={{ display: 'inline-flex', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .14s ease' }}>
          {Ic('chevron-down', 'ico-xs')}
        </span>
      </button>
...
export default function Opportunities({ onOpen, onNew, onEdit, perms = {} }) {
  ...
  const toggle = (k, o) => setSel((s) => {
    const arr = s[k];
    const next = arr.includes(o) ? arr.filter((x) => x !== o) : arr.concat(o);
    return { ...s, [k]: next };
  });
  const radio = (k, v) => setSel((s) => ({ ...s, [k]: v }));
  ...
  const rows = D.opportunities.filter((o) => {
    ...
    if (sel.tipo.length && !sel.tipo.includes(o.tipo)) return false;
    if (sel.custo.length && !sel.custo.includes(o.custo)) return false;
            ...
            <div style={{ padding: 48, textAlign: 'center', color: 'var(--muted-foreground)' }}>
              ...
              {(activeCount > 0 || q) && (
                <div style={{ marginTop: 14 }}>
                  <Button variant="outline" size="sm" onClick={clear}>Limpar filtros</Button>
                </div>
              )}
            </div>
          </Card>
